...
#define FOR_EACH(it, range) \
    for (let it = (range).data; it < (range).data + (range).count; ++it)
...
World makeWorld() {
    ...
    world.lights.data[0] = (Light){(Vec3d){+1, +1, +2}, 0.4 * (Vec3d){1, 0.8, 0.5}};
    world.lights.data[1] = (Light){(Vec3d){-1, -1, -2}, 0.4 * (Vec3d){0.5, 0.5, 1}};
